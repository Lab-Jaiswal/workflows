#this is a reference file. Don't change!
...
#[Merge sample bam files for each replicate condition to single bam file for each condition (using samtools merge)]
...
#Basically Combine bam files of replicate samples
#by joining individual bamfiles from the same condition
