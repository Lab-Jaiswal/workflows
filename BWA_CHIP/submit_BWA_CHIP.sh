...
if [ -z "$1" ] || [ -z "$2" ] || [ -z "$3" ]; then
    ...
    TEMP=`getopt -o vdm: --long min_coverage:,min_var_freq:,p_value:,intervals:,tumor_normal:,log_name:,bwa_gref:,twist_snps:,assembly:,funcotator_sources:,transcript_list:,bam,twist,filtered,mutect,varscan,haplotypecaller,all \
   ...
   if [ $? != 0 ]; then
    ...
    while true; do
        case "$1" in
            ...
            --tumor_normal ) tumor_normal=true; normal_sample="$2"; shift 2 ;; 
