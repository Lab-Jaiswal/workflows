...
function run_job() {
        ...
        if [[ $panel_of_normals == false ]]; then
                ...
                batch_size=1   
                array_number=1
                filtered_list_of_samples=false
        fi
...
function main() {
        ...
        for i in $(seq 1 ${number_of_batches}); do
               ...
               job_id=$(dx-jobutil-new-job run_job -iarray_number=${i} -ibatch_size=${batch_size} -ipanel_of_normals=${panel_of_normals} --instance-type mem3_ssd1_v2_x96)
               echo "job_id: $job_id"
