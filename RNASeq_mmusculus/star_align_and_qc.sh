...
#star_index="/ifs/labs/sjaiswal/genomes/mm10/STAR_index/" #path to STAR genome index
#annotation_gtf="/ifs/labs/sjaiswal/genomes/mm10/gencode.vM20.annotation.gtf" #path to annotation GTF

#reference_genome="/ifs/labs/sjaiswal/genomes/mm10/GRCm38.primary_assembly.genome.fa" #location of reference genome used in alignment
#annotation_refFlat="/ifs/labs/sjaiswal/genomes/mm10/gencode.vM20.primary_assembly.annotation.fixed.refFlat.txt" ## Created from ensembl GTF using gtfToGenePred -genePredExt -geneNameAsName2 from bioconda

module load star #Load STAR into path
module load picard #Load Picard into path
module load fastqc #Load FastQC into path

source "/labs/sjaiswal/workflows/RNASeq_mmusculus/workflow_config.sh" #Load configuration with reference file locations
...
fastq_file="${output_directory}/fastq_files" #provide path to file containing list of fastq files
