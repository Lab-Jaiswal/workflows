...
genekey_name <- read_lines("/labs/sjaiswal/workflows/RNASeq_mmusculus/workflow_config.sh")  %>%
    str_subset("gene_key") %>%
    str_replace_all(" #.*$", "") %>%
    str_replace_all("\\\"", "") 
genekey_path <- str_split(genekey_name, "=")[[1]][2]
gtf_names <- read_tsv(genekey_path) #Load gene key (generated from GTF using create_genekey.R)

count_files <- list.files(str_c(project_directory, "/alignment_output/"), pattern = "*ReadsPerGene*", full.names = TRUE) #Get names of all of files containing counts for individual samples
sample_names <- map_chr(count_files, basename) %>% str_replace_all("_L[0-9]{3}.*$", "") %>% str_replace_all("\\-", "_") %>% str_replace_all("\\-", " ") %>% make.names #Get samples names from file names
read_files <- map(count_files, read_tsv, col_names = FALSE) %>% #Read in each tab separated file
    ...
    filter(grepl("^ENSMUSG", Ensembl_ID)) %>% #Remove rows that do not contain gene IDs in Ensembl_ID column
...
write_tsv(read_files, str_c(project_directory, "/counts.txt")) #Write final tibble to disk as tsv file
...
#normalized_expr <- data.frame(Ensembl_ID = read_files$Ensembl_ID, Symbol = read_files$Symbol, vst_normalized, stringsAsFactors = FALSE) %>% #Put normalized expression matrix back into data frame with gene IDs and symbols
    #mutate_if(is.numeric, round, digits = 3) #Round normalized values to 3 digits to reduce file size on disk
...
#expr_DESeqDataset <- DESeqDataSetFromMatrix(countData = expr_matrix, design = ~1, colData = coldata, tidy = FALSE) 
