...
#genekey_name <- read_lines("/labs/sjaiswal/workflows/RNASeq_hsapiens/workflow_config.sh")  %>%
    #str_subset("gene_key") %>%
    #str_replace_all(" #.*$", "") %>%
    #str_replace_all("\\\"", "") 
...
read_files <- map(count_files, read_tsv, col_names = FALSE) %>% #Read in each tab separated file
    ...
    select(Ensembl_ID, !!first(sample_names):!!last(sample_names)) #Reorder columns so that Ensembl_ID and Symbol are first, followed by data
