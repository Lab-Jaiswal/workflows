...
if [ -z "$1" ]; then
    ...
    code_directory="/labs/sjaiswal/workflows/RNASeq_hsapiens" #specify location of star_align_and_qc.sh
    ...
    sbatch -o "${output_directory}/alignment_log" `#put standard output into log` \
        -e "${output_directory}/alignment_log" `#put standard error into log` \
